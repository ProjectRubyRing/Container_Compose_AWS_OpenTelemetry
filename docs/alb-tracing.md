...
```sh
# 帳票 EC2 の起動スクリプト
export OTEL_SERVICE_NAME=ec2_server                    # ★ 下の注意を読むこと
...
違う文字列だと `ec2_server` と `report-alb` のような 2 ノードが
並んで出る。
...
| 設定 | 値 |
|---|---|
| EC2 の `OTEL_SERVICE_NAME` | `ec2_server` (= `PEER_NAME_EC2` の値) |
| コンテナ側 `APP_PEER_ALB_MODE` | `upstream` (既定) → `peer.service` = `ec2_server` |

ノード名は `PEER_NAME_EC2` で変えられる (→ [`xray-node-naming.md`](xray-node-naming.md))。
変えたら EC2 側の `OTEL_SERVICE_NAME` も同じ文字列に直すこと。

**`alb` モードと EC2 の計装は併用しない。**
`alb` モードにすると `peer.service` が `report-alb` になり、
EC2 の `ec2_server` と一致しなくなる。
...
これで `front ──▶ ec2_server` というエッジ 1 本しか無くても、
...
利用者 / EC2バッチ                 ALB                     ECS タスク
    ...
    │                              │   X-Amzn-Trace-Id を送出  │  peer.service=ec2_server
    │                              │ ────────────────────────▶│  app_via=alb
    │                              │        帳票 EC2           │  app_upstream=…
```
...
alb_trace_id=Root=1-...
app_peer=ec2_server        (APP_PEER_ALB_MODE=alb なら report-alb)
```

`APP_PEER_ALB_MODE` を切り替えて `app_peer` が
`ec2_server` ↔ `report-alb` と変わることも確認できる。
