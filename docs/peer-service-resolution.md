...
```
intra-api-back ──▶ DataBase（Aurora_MySQL）
               ──▶ session_store（Valkey）
               ──▶ ec2_server   (ALB モードが alb なら report-alb)
```
...
`DataBase（Aurora_MySQL）` のままノード名が保たれる。ドメイン判定を先にすると、
...
**表示名は `otel-env.sh` 6-1 節の `PEER_NAME_*` が唯一の決定箇所**で、
名前を変えたいときはそこ (または同名の環境変数) だけを触る。
→ [`xray-node-naming.md`](xray-node-naming.md)

| 環境変数 | 表示名を決める変数 | 既定の論理名 |
|---|---|---|
| `DB_HOST` | `PEER_NAME_AURORA` | `DataBase（Aurora_MySQL）` |
| `VALKEY_HOST` | `PEER_NAME_VALKEY` | `session_store（Valkey）` |
| `REPORT_ALB_HOST` (or `REPORT_ALB_URL`) | `PEER_NAME_EC2` / `PEER_NAME_ALB` | `ec2_server` / `report-alb` |
| `EXTERNAL_SLB_HOST` (or `EXTERNAL_SLB_URL`) | `PEER_NAME_EXTERNAL_SLB` | `external-slb` |
| `SQS_HOST` (or `SQS_ENDPOINT`) | `PEER_NAME_SQS` | `sqs` |
| `BACKEND_HOST` (or `BACKEND_URL`) ※front のみ | `PEER_NAME_BACKEND` | `<APP_SERVICE>-back` |
...
```sh
# 既定値 (論理名は PEER_NAME_* を展開したもの)
APP_PEER_DOMAIN_RULES=.rds.amazonaws.com=DataBase（Aurora_MySQL）,.cache.amazonaws.com=session_store（Valkey）,.elb.amazonaws.com=alb,sqs.=sqs,.s3.=s3,s3.=s3,.execute-api.=api-gateway,.secretsmanager.=secretsmanager,.dkr.ecr.=ecr
```

★ 段1 と段2 で同じ相手に違う名前を付けると、マップ上で 2 ノードに割れる。
既定値が `PEER_NAME_*` を参照しているのはそのため。ここを明示指定すると
その連動が切れるので、`PEER_NAME_*` を変えたときは両方そろえること。
...
```sh
# 既定値 (論理名は PEER_NAME_* を展開したもの)
APP_PEER_PORT_RULES=3306=DataBase（Aurora_MySQL）,6379=session_store（Valkey）,4317=adot-collector,4318=adot-collector,2000=adot-awsproxy
```
...
[otel-env]       DB_HOST=aurora:3306 -> DataBase（Aurora_MySQL） (段: role)
[otel-env]       VALKEY_HOST=valkey:6379 -> session_store（Valkey） (段: role)
[otel-env]       REPORT_ALB_HOST=alb:80 -> ec2_server (段: role)
...
```
[otel-env]       DB_HOST=aurora-prd.cluster-cxa1b2.ap-northeast-1.rds.amazonaws.com:3306 -> DataBase（Aurora_MySQL） (段: role)
[otel-env]       10.0.3.21:6379 -> session_store（Valkey） (段: port)
