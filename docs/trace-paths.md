...
(名前は `PEER_NAME_EC2`。`APP_PEER_ALB_MODE=alb` にすると `PEER_NAME_ALB`
= 既定 `report-alb`)。
...
```sh
export OTEL_SERVICE_NAME=ec2_server                  # ★ peer.service と同じ文字列に
...
`ec2_server` と `report-alb` のように同じ相手が 2 ノードに割れる。
