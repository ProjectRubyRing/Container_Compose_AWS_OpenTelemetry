...
#     段1  役割     変数名から用途が確定しているもの
#                   (DB_HOST -> PEER_NAME_AURORA)
#     段2  ドメイン ホスト名の部分一致
#                   (.rds.amazonaws.com -> PEER_NAME_AURORA)
#     段3  ポート   ポート番号 (3306 -> PEER_NAME_AURORA)
#     段4  AWS 自動 *.<service>.amazonaws.com からサービス名を機械的に抜く
#     (どれも当たらなければマッピングを出さない = FQDN のまま出る)
#
#   ★ 段1〜段3 が返す表示名は 6-1 節の PEER_NAME_* に集約してある。
#     どの段で決まっても同じノード名になるようにするため。
#     (既定値と変更手順: docs/xray-node-naming.md)
#
...
#                     例: REPORT_ALB_HOST -> PEER_NAME_EC2 (既定 ec2_server)
#                     マップは front -> ec2_server の 1 エッジになる。
#     alb             ALB 自体をノードにする。
...
#
#   論理名をリテラルで埋め込まず変数にしてあるのは、「このノードの名前を
#   変えたい」と思ったときに探す場所を 1 か所に固定するため。値はすべて
#   環境変数 (compose / タスク定義) から上書きできる。再ビルドは不要。
#
#   ★ 段2 (ドメイン) / 段3 (ポート) の既定ルールもこの変数を参照している。
#     同じ Aurora を段1 で拾ったスパンと段2 で拾ったスパンがあっても、
#     名前が割れてマップが 2 ノードに分裂しないようにするため。
#     Collector 側 (otel/collector-*.yaml の transform/peer-service-resolve)
#     にも同じ文字列が入っている。ここを変えたら必ず両方を直し、
#     scripts/check-collector-drift.sh で一致を確認すること。
#
...
#       段2/段3 の既定ルール (APP_PEER_DOMAIN_RULES / APP_PEER_PORT_RULES)
#       も同じ区切り文字を使うので、そちらも同時に壊れる。
#
#   (a2) 空白  ->  警告だけ
#       X-Ray のセグメント名としては空白は使える。ただし段2/段3 のルールは
#       シェルの単語分割で要素を切るため、既定ルールに埋め込まれると
#       そこで名前が途切れる (段1 = peer-service-mapping 側は無事)。
#       空白を使いたいときは APP_PEER_DOMAIN_RULES / APP_PEER_PORT_RULES を
#       明示指定して、段2/段3 では別名を使うこと。
#
...
otel_peer_name_check() {
    ...
    # (a2) 段2/段3 のルール文字列は空白で要素が切れる
    case "${_pn_val}" in
        *" "*) otel_warn "${_pn_name} に空白が含まれています: [${_pn_val}] / 段1 (peer-service-mapping) では問題ありませんが、段2/段3 の既定ルールはここで名前が途切れます。APP_PEER_DOMAIN_RULES / APP_PEER_PORT_RULES を明示指定してください。" ;;
    esac
...
for _pn in PEER_NAME_AURORA PEER_NAME_VALKEY PEER_NAME_EC2 PEER_NAME_ALB \
...
#   ★ 論理名は 6-1 節の PEER_NAME_* を参照する。段1 で拾えたスパンと
#     段2 で拾えたスパンが同じ相手を別名で呼び、マップ上で 2 ノードに
#     割れるのを防ぐため。
: "${APP_PEER_DOMAIN_RULES:=.rds.amazonaws.com=${PEER_NAME_AURORA},.cache.amazonaws.com=${PEER_NAME_VALKEY},.elb.amazonaws.com=alb,sqs.=${PEER_NAME_SQS},.s3.=s3,s3.=s3,.execute-api.=api-gateway,.secretsmanager.=secretsmanager,.dkr.ecr.=ecr}"
...
: "${APP_PEER_PORT_RULES:=3306=${PEER_NAME_AURORA},6379=${PEER_NAME_VALKEY},4317=adot-collector,4318=adot-collector,2000=adot-awsproxy}"
...
#   ★ 表示名はリテラルではなく 6-1 節の PEER_NAME_* を使う。
#     ノード名の決定箇所を 1 か所に保つため。
...
EXTERNAL_SLB_HOST|EXTERNAL_SLB_PORT|EXTERNAL_SLB_URL|${PEER_NAME_EXTERNAL_SLB}|外部 SLB (VPC 外)
...
TARGETS
    ...
    if [ "${APP_ROLE}" = "front" ]; then
        echo "BACKEND_HOST|BACKEND_PORT|BACKEND_URL|${PEER_NAME_BACKEND}|同一タスク内の back"
    fi
}
