...
docs/
  ...
  xray-node-naming.md        ★ 下流ノードの表示名の決め方 (PEER_NAME_*)
...
**アプリのコードは 1 行も触らない** — すべて `otel-env.sh` が環境変数から
組み立てる。
...
**表示名そのものは `otel-env.sh` 6-1 節の `PEER_NAME_*` 1 か所に集約**して
あり、段1〜段3 のどれで決まっても同じ名前になる (`DataBase（Aurora_MySQL）` /
`session_store（Valkey）` / `ec2_server` / `external-slb`)。
X-Ray のセグメント名に使えない文字 (丸括弧など) が入っていないかは起動時に
検査し、`APP_PEER_NAME_MODE` で警告 / 自動置換 / 起動拒否を選べる。
→ 詳細: [`docs/xray-node-naming.md`](docs/xray-node-naming.md)

**どの段で決まったかを残すのがこの仕組みの肝。**

```sh
docker compose logs front | grep -A 8 "peer 判定"
#   DB_HOST=aurora:3306 -> DataBase（Aurora_MySQL） (段: role)
#   10.0.3.21:6379      -> session_store（Valkey） (段: port)
```
